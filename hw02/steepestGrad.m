...
%         converges to the local minima using this approach. Not sure if it is the
%         correct way. 
%         The statement below can be used instead of the loop to consider
%         this way. 
...
%         alpha{k+1} = alpha_new{i}; %update alpha with the one found using NM
        X_init_array1{k+1}= X_init_array1{k}-alpha{k}*grad{k}; %update point using the new found alpha
        k = k+1;
    end
end